...
void main() {
  group('normalizePathSeparators', () {
    test('Should replace backslashes with forward slashes', () {
      expect(
        r'lib\core\utils'.normalizePathSeparators(pathSeparator: '/'),
        'lib/core/utils',
      );
    });
    ...
    test('Should replace forward slashes with forward slashes (no-op)', () {
      expect(
        'lib/core/utils'.normalizePathSeparators(pathSeparator: '/'),
        'lib/core/utils',
      );
    });
    ...
    test('Should handle a single backslash', () {
      expect(r'a\b'.normalizePathSeparators(pathSeparator: '/'), 'a/b');
    });

    test('Should handle an empty string', () {
      expect(''.normalizePathSeparators(pathSeparator: '/'), '');
    });

    test('Should handle a string with no separators', () {
      expect(
        'filename.dart'.normalizePathSeparators(pathSeparator: '/'),
        'filename.dart',
      );
    });
    ...
    test('Should handle trailing separator', () {
      expect(
        'lib/core/'.normalizePathSeparators(pathSeparator: '/'),
        'lib/core/',
      );
    });
  ...
  group('ensureTrailingPathSeparator', () {
    test('Should add trailing slash when missing', () {
      expect(
        'lib/core'.ensureTrailingPathSeparator(pathSeparator: '/'),
        'lib/core/',
      );
    });
    ...
    test('Should handle empty string', () {
      expect(''.ensureTrailingPathSeparator(pathSeparator: '/'), '/');
    });

    test('Should handle single character string', () {
      expect('a'.ensureTrailingPathSeparator(pathSeparator: '/'), 'a/');
    });
  });

  group('surroundingPathSeparator', () {
    test('Should add leading and trailing slashes when neither present', () {
      expect('domain'.surroundingPathSeparator(pathSeparator: '/'), '/domain/');
    });

    test('Should add leading slash only', () {
      expect(
        'domain/'.surroundingPathSeparator(pathSeparator: '/'),
        '/domain/',
      );
    });
    ...
    test('Should handle empty string', () {
      expect(''.surroundingPathSeparator(pathSeparator: '/'), '/');
    });

    test('Should handle nested path', () {
      expect(
        'lib/core'.surroundingPathSeparator(pathSeparator: '/'),
        '/lib/core/',
      );
    });
  });
