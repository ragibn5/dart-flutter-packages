...
class ShowAppNameChangeGuide {
  ...
  const ShowAppNameChangeGuide({Prompter prompter = const ConsolePrompter()})
      : _prompter = prompter;

  /// Prints Android `strings.xml` and iOS Xcode build-setting instructions
  /// for [projectPath], then blocks until Enter is pressed.
  ///
  ...
  Future<void> call(String projectPath) async {
    _prompter
      ..write(
        ...
        'value:\n'
        '     - $projectPath/android/app/src/dev/res/values/strings.xml   '
        "(for the 'dev' flavor)\n"
        '     - $projectPath/android/app/src/exp/res/values/strings.xml   '
        "(for the 'exp' flavor)\n"
        '     - $projectPath/android/app/src/stage/res/values/strings.xml '
        "(for the 'stage' flavor)\n"
        '     - $projectPath/android/app/src/prod/res/values/strings.xml  '
        "(for the 'prod' flavor)\n"
