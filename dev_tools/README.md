...
| `whitelabel create`                  | Copies the current Flutter template (or `--template <dir>`) to `<destination>`, excluding `.git` and project-defined paths. The destination must be new, or empty with `--force`. |
