...
class AppLocaleResolver {
  /// Resolves an [AppLocale] from a [LocaleComponents] if possible, or null.
  AppLocale? resolverAppLocale(LocaleComponents requested) {
    bool match(AppLocale locale, {bool script = false, bool country = false}) {
      if (locale.languageCode != requested.languageCode) {
        return false;
      }
      if (script && locale.scriptCode != requested.scriptCode) {
        return false;
      }
      if (country && locale.countryCode != requested.countryCode) {
        return false;
      }
      return true;
    }
    ...
    for (final locale in AppLocale.values) {
      if (match(locale, script: true, country: true)) {
        return locale;
      }
    }
